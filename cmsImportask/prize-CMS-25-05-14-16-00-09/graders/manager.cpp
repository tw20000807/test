...
void quitf(double p, string r = ""){
  cout << p << endl;
  cerr << r << endl;
}
void qp(double grade) {
  quitf(grade/double(80));
}
inline void finish() {
  int Q = queries.size();
		if(subtask_type == "v2") {
			if(10000 < Q) quitf(0, "too many queries");
			quitf(1);
		}
		else if(subtask_type == "no_limit") {
			if (10000 < Q)	quitf(0, "too many queries");
			if (6000 < Q)	qp(70.0);
			...
			quitf(0, "unknown subtask");
}

inline void doubleWrite(double code, string msg = "") {
  cout << code << endl;
  cerr << msg << endl;
}
...
int main(int argc, char **argv) {
    ...
    if (strategy_type == "plain") {
        ...
        doubleWrite(0, "invalid strategy type: "+strategy_type);
        exit(0);
    }
    ...
    while (true) {
	  ...
	  int index = readValidIndex("B",false);
	    answer = index;
	        ...
	        doubleWrite(0, "answer is not correct");
            }
	    finish();
            exit(0);
        } else {
